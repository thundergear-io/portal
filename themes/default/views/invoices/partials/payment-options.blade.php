<div>
    ...
    <div class="mb-6">
        ...
        <div wire:click="$set('selectedMethod', 'credit')"
            ...
            class="flex items-center justify-between p-4 bg-base-100 border border-base-300 rounded-3xl cursor-pointer transition-all {{ $selectedMethod === 'credit' ? 'border-primary ring-2 ring-primary' : 'border-base-300 hover:border-primary/50' }}">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-base-content/50">{{ __('invoices.available_credits', ['amount' => $credit->formattedAmount]) }}</p>
                </div>
            </div>
            <div
                class="size-5 rounded-full bg-base-100 border border-base-300 flex items-center justify-center {{ $selectedMethod === 'credit' ? 'border-primary bg-primary' : 'border-base-300' }}">
    ...
    <div class="mb-6">
        ...
        <div class="space-y-3">
            ...
            <div wire:click="$set('selectedMethod', '{{ $method->ulid }}')"
                class="flex items-center justify-between p-4 bg-base-100 border rounded-3xl cursor-pointer transition-all
                    {{ $selectedMethod === $method->ulid ? 'border-primary ring-2 ring-primary' : 'border-base-300 hover:border-primary/50' }}"
                    wire:loading.class="opacity-50 pointer-events-none" wire:target="selectedMethod,processPayment">
                ...
                <div
                    class="size-5 rounded-full border border-base-300 flex items-center justify-center
                                    {{ $selectedMethod === $method->ulid ? 'border-primary bg-primary' : 'border-base-300' }}">
    ...
    <div class="mb-6" x-data="{ showOneTime: @if($this->savedPaymentMethods->count() == 0) true @else false @endif }">
        <button @click="showOneTime = !showOneTime"
            class="flex items-center justify-between w-full p-3 text-left rounded-3xl border border-base-300 transition-colors @if($this->savedPaymentMethods->count() == 0) hidden @endif">
        ...
        <div class="space-y-3 mt-3" x-show="showOneTime" x-transition>
            ...
            <div wire:click="$set('selectedMethod', 'gateway-{{ $method->id }}')"
                class="flex items-center justify-between p-4 border rounded-3xl cursor-pointer transition-all {{ $selectedMethod === 'gateway-' . $method->id ? 'border-primary ring-2 ring-primary' : 'border-base-300 hover:border-primary/50' }}"
                wire:loading.class="opacity-50 pointer-events-none" wire:target="selectedMethod,processPayment">
                <div class="flex items-center space-x-4">
                    <div
                        class="bg-base-100 border border-base-300 rounded-3xl overflow-hidden flex items-center justify-center h-9 w-9">
                        @if($method->meta?->icon)
                    ...
                    <div>
                        ...
                        <p class="text-sm text-base-content/50">{{ __('invoices.one_time_payment') }}</p>
                    </div>
                </div>
                <div
                    class="size-5 rounded-full bg-base-100 border border-base-300 flex items-center justify-center {{ $selectedMethod === 'gateway-' . $method->id ? 'border-primary bg-primary' : 'border-base-300' }}">
