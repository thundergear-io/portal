...
<head>
    ...
    @livewireStyles
    @vite(['themes/' . config('settings.theme', 'default') . '/js/app.js', 'themes/' . config('settings.theme', 'default') . '/css/app.css'], config('settings.theme', 'default'))
...
<body class="w-full bg-base-100 text-base-content min-h-screen flex flex-col antialiased" x-cloak x-data="{darkMode: $persist(window.matchMedia('(prefers-color-scheme: dark)').matches)}" :class="{'dark': darkMode}" :data-theme="darkMode ? 'dark' : 'light'">
    {!! hook('body') !!}
    <x-navigation />
