<div class="lg:px-4 lg:py-6 flex flex-col gap-2">
    <div class="flex flex-col gap-2 md:hidden">
        ...
        <div x-data="{ activeAccordion: {{ $nav['active'] ? 'true' : 'false' }} }"
            class="relative w-full mx-auto overflow-hidden text-base font-normal divide-y divide-gray-200">
            <div class="cursor-pointer">
                <button @click="activeAccordion = !activeAccordion"
                    class="flex items-center justify-between w-full p-3 text-base whitespace-nowrap rounded-3xl hover:bg-primary/5">
                    <div class="flex flex-row gap-2">
                        @isset($nav['icon'])
                            <x-dynamic-component :component="$nav['icon']"
                            class="size-5 {{ $nav['active'] ? 'text-primary' : 'text-accent' }}" />
                        @endisset
                        <span>{{ $nav['name'] }}</span>
                    </div>
                ...
                <div x-show="activeAccordion" x-collapse x-cloak>
                    <div class="p-4 pt-0 opacity-70 text-base">
        ...
        @else
        <div class="flex items-center rounded-3xl {{ $nav['active'] ? 'bg-primary/5' : 'hover:bg-primary/5' }}">
            ...
            <x-dropdown>
                <x-slot:trigger>
                    <div class="flex flex-col">
                        <span class="text-base text-nowrap">{{ strtoupper(app()->getLocale()) }} <span class="text-base/50">|</span> {{ session('currency', config('settings.default_currency')) }}</span>
                    </div>
                </x-slot:trigger>
                <x-slot:content>
