...
<label for="{{ $id }}" class="flex items-center {{ $disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer' }}">
    <div class="relative">
        <input
            ...
            {{ $disabled ? 'disabled' : '' }}
        >
        <div class="w-11 h-6 bg-slate-900 rounded-full peer-checked:bg-accent transition-colors duration-300 ease-in-out"></div>
    ...
    @isset($label)
    <span class="ml-3 text-sm font-medium text-base-content/80">{{ $label }}</span>
    @endisset
</label>
