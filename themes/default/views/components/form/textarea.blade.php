...
<fieldset class="flex flex-col w-full {{ $divClass ?? '' }}">
    ...
    <textarea type="{{ $type ?? 'text' }}" id="{{ $id ?? $name }}" name="{{ $name }}"
        class="block w-full text-md text-base-content bg-base-300 border border-base-200 rounded-2xl outline-none focus:outline-none transition-all duration-300 ease-in-out disabled:bg-base-300/50 disabled:cursor-not-allowed {{ $class ?? '' }} @if ($type !== 'color') px-4 py-4 @endif"
