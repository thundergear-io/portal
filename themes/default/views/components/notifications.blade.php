<div>
    ...
    <x-dropdown width="w-84" :showArrow="false">
        <x-slot:trigger>
            <div class="relative w-10 h-10 flex items-center justify-center rounded-3xl hover:bg-neutral transition" x-data="{ hasNew: false }" x-on:new-notification.window="hasNew = true"
                @click="hasNew = false">
        ...
        <x-slot:content>
            <div class="w-full max-h-96 overflow-y-auto">
                ...
                <div wire:click="goToNotification({{ $notification->id }})"
                    class="block px-4 py-3 hover:bg-base-100/50 cursor-pointer @if (!$loop->last) border-b border-base-300/50 @endif">
