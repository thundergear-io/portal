...
<div class="relative" x-data="{ open: false, adjustWidth: 0 }" x-init="$watch('open', value => {
    ...
    <button
        class="flex flex-row items-center px-2 py-1 whitespace-nowrap text-base-content hover:text-base-content/80"
        x-on:click="open = !open">
        {{ $trigger }}
        @if($showArrow)
        <x-ri-arrow-down-s-line x-bind:class="{ '-rotate-180' : open }"
            class="md:block hidden size-4 text-base-content ease-out duration-300" />
        @endif
    </button>

    <div x-ref="dropdown"
        class="absolute mt-2 {{ $width ?? "w-48" }} px-4 py-3 bg-base-100 rounded-3xl shadow-lg z-10 border border-base-300"
        x-bind:style="{
            left: `-${adjustWidth}px`,
        }"
        x-show="open"
